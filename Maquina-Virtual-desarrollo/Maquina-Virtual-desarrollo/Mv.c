...
void ejecuta_instruccion(char MP[], int registros[], int tabla_segmentos[], Tmnemonicos VMnemonicos[])
{
    ...
    if (registros[IP] <= registros[DS] + (tabla_segmentos[0] & 0x00FF))
    { // suponiendo que el CS siempre esta en el 0 de la tabla de segmentos
...
void inicializa_registros(int tabla_segmentos[], int registros[])
{
    ...
    /*
    registros[CS] = 0 << 16;   // en registros cs se almacena en los 16 bits mas significativos la posicion en la tabla de segmentos
    registros[DS] = 1 << 16;  // lo mismo para el ds
...
void leer_codigo(char MP[MAX_MEMORIA], int tabla_segmentos[], char nombreArch[])
{
    ...
    if (Ar != NULL)
        while (fread(&x, sizeof(char), 1, Ar) == 1)
        {
            ...
            else if (strcmp(identificador, "VMX26") != 0 || version != 1) // le agrego si version es != 1 por si las dudas
                break;
            else
                MP[i] = x;
            i++;
